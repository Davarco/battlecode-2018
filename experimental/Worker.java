...
public class Worker {
    ...
    private static boolean moveTowardsRocket() {
        ...
        if (idx != -1) {
            ...
            if (map.onMap(tmp)) {
                if (!EarthPathing.move(worker, tmp)) {
                    EarthPathing.tryMove(worker, worker.location().mapLocation().directionTo(tmp));
                }
                return true;
            }
            tmp = new MapLocation(Planet.Earth, initx - 1, inity);
    ...
    private static boolean moveTowardsKarboniteFar() {
        if (gc.planet().equals(Planet.Earth)) {
            ...
            EarthPathing.tryMove(worker, worker.location().mapLocation()
            ...
            MarsPathing.tryMove(worker, worker.location().mapLocation()
    ...
    private static boolean moveTowardsKarbonite() {
        ...
        if (bestKarb != null) { // bestKarboniteLoc returns the worker's position if nothing is found
            if (EarthPathing.move(worker, bestKarb) == false) {
                EarthPathing.tryMove(worker, worker.location().mapLocation().directionTo(bestKarb));
            }
            return true;
        }
    ...
    private static boolean ditchFactory() {
        ...
        Direction opposite = EarthPathing.opposite(worker.location().mapLocation().directionTo(units.get(idx).location().mapLocation()));
        EarthPathing.tryMove(worker, opposite);
        return true;
    }
    ...
    private static boolean bounce() {
        ...
        if (dir != null) {
            ...
            if (EarthPathing.tryMove(worker, dir)) {
    ...
    private static boolean returnToFactory() {
        ...
        EarthPathing.tryMove(worker, worker.location().mapLocation().directionTo(units.get(idx).location().mapLocation()));
        return true;
    }
    ...
    private static void harvestKarbonite() {
        for (Direction d : Direction.values()) {
            if (gc.startingMap(worker.location().mapLocation().getPlanet()).onMap(worker.location().mapLocation().add(d)) && gc.canHarvest(worker.id(), d)) {
                ...
                MapLocation ml = EarthPathing.DirectionToMapLocation(worker, d);
                if (worker.location().mapLocation().getPlanet().equals(Planet.Earth)) {
