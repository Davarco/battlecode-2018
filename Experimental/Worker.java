   ...
   /* private static boolean moveTowardsKarbonite() {
    ...
    private static boolean moveTowardsKarbonite() {
    		if(gc.planet().equals(Planet.Earth)) {
    			...
    			Pathing.tryMove(worker, worker.location().mapLocation().directionTo(Mars.karboniteplacesEarth.get(idx).get(earthkarboindex.get(idx))));
    			return true;
    		}
    		else {
    			int idx = Mars.marsplaces[worker.location().mapLocation().getX()][worker.location().mapLocation().getY()];
    			...
    			Pathing.tryMove(worker, worker.location().mapLocation().directionTo(Mars.karboniteplacesMars.get(idx).get(marskarboindex.get(idx))));
    			return true;
    		}
    }
