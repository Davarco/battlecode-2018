...
public class Mage {
	...
	private static void moveSmall() {
		...
		/*
		 * TODO Implement the entire worker changes function as a heuristic
		 * based on priority
		 */
		...
		if (Player.focalPoint != null) {
			if (Player.focalPoint.isWithinRange(mage.visionRange(), mage.location().mapLocation())
					&& gc.canSenseLocation(Player.focalPoint) && !gc.hasUnitAtLocation(Player.focalPoint)) {
		...
		// If none of the above work, changes in a random direction (placeholder
		// for now)
	...
	private static boolean rush() {
		...
		if (enemies == null || enemies.size() == 0) {
			if (!Pathing.move(mage, Player.enemy)) {
				return Pathing.tryMove(mage, mage.location().mapLocation().directionTo(Player.enemy));
			}
			return false;
		}
		for (int x = 0; x < enemies.size(); x++) {
			if (enemies.get(x).unitType().equals(UnitType.Factory)) {
				if (!Pathing.move(mage, enemies.get(x).location().mapLocation())) {
					return Pathing.tryMove(mage,
							mage.location().mapLocation().directionTo(enemies.get(x).location().mapLocation()));
				}
			}
		...
		if (!Pathing.move(mage, enemies.get(0).location().mapLocation())) {
			return Pathing.tryMove(mage,
					mage.location().mapLocation().directionTo(enemies.get(0).location().mapLocation()));
		}
		return true;
	}

	private static boolean moveTowardsRocketSmall() {
		if (mage.location().mapLocation().getPlanet() == Planet.Mars)
			return false;
	...
	private static void move() {
		...
		/*
		 * if (gc.round() < Config.mage_AUTO_ATTACK_ROUND &&
		 * Info.number(UnitType.mage) >= 8 &&
		 * mage.location().isOnPlanet(Planet.Earth)) if (moveTowardsInitPoint())
		 * return;
		 */
	...
	private static void moveMars() {
		...
		if (Player.focalPointMars != null) {
			if (Player.focalPointMars.isWithinRange(mage.visionRange(), mage.location().mapLocation())
					&& gc.canSenseLocation(Player.focalPointMars) && !gc.hasUnitAtLocation(Player.focalPointMars)) {
				// System.out.println("Works");
				Player.focalPoint = null;
			}
		}
		...
		// Move towards rockets mid-game, and escape factories early on
		if (mage.location().isOnPlanet(Planet.Earth) && gc.round() >= Config.ROCKET_CREATION_ROUND
				&& mage.location().mapLocation().getPlanet() == Planet.Mars) {
			if (moveTowardsRocket()) {
				return;
			}
		}
	...
	private static boolean bounce() {
		...
		if (counterMap.get(mage.id()) >= 8) {
			...
			for (Direction d : Direction.values()) {
				...
				if (gc.startingMap(mage.location().mapLocation().getPlanet()).onMap(loc)
						&& (gc.startingMap(mage.location().mapLocation().getPlanet()).isPassableTerrainAt(loc) == 1)
	...
	private static boolean ditchFactory() {
		...
		Direction opposite = Pathing
				.opposite(mage.location().mapLocation().directionTo(units.get(idx).location().mapLocation()));
	...
	private static boolean ditchRocket() {
		...
		int idx = 0;
		for (int i = 0; i < units.size(); i++) {
			if (units.get(i).structureIsBuilt() == 1)
				continue;
			long dist = 50 - mage.location().mapLocation().distanceSquaredTo(units.get(i).location().mapLocation());
			if (dist > maxDist && units.get(i).health() == units.get(i).maxHealth()) {
				maxDist = dist;
